...
const notification = require('./components/tme_notifier')
...
let budget = conf.get('nbt.budget')?parseFloat(conf.get('nbt.budget')):10.10
...
console.log('')
  ...
  let buy_info_request = [
    {
      type: 'input',
      name: 'base_currency',
      message: chalk.cyan('Base-Currency (USDT, BTC, BNB, ETH)'),
    ...
    },
    {
      ...
      name: 'budget',
      default: budget,
      message: chalk.cyan('Treade budget (in base currency)(total value. > 14 USDT)'),
      validate: function(value) {
  ...
  ask_buy_sell_options = () => {
    ...
    inquirer.prompt(buy_sell_options).then(answers => {
      // ans.buy_option  == Buy at Market Price (не понятно)
      if (answers.buy_option.includes("Market")) {
        // console.log('---',answers.buy_option.includes(''))
        // MARKET PRICE BUY //
  ...
  ask_loss_profit_percents = () => {
    ...
    inquirer.prompt(loss_profit_inputs).then(answers => {
      if (answers.confirm) {
        let pc = conf.set('nbt.profit_pourcent', answers.profit_pourcent)
  ...
  start_trading = () => {
    ...
    if (buying_method === "Fixed") {
      ...
      notification('(' + currency_to_buy + ') ' + "ПОКУПКА. Цена: " + buy_amount + " AT FIXED PRICE ")
      client.order({
        symbol: pair,
        side: 'BUY',
        quantity: buy_amount,
    ...
    if (buying_method === "Bid") {
      ...
      notification('(' + currency_to_buy + ') ' + "ПОКУПКА. Цена: " + buy_amount  + "1ST BID ")
      client.order({
        symbol: pair,
        side: 'BUY',
        quantity: buy_amount,
    ...
    if (buying_method === "Market") {
      ...
      console.log(chalk.green('(' + currency_to_buy + ') ' + "ПОКУПКА. Цена: " + buy_amount  + " AT MARKET PRICE" ))
      // notification("BUYING " + buy_amount + " OF " + currency_to_buy + " AT MARKET PRICE" )
      client.order({
        symbol: pair,
        side: 'BUY',
        quantity: buy_amount,
      ...
      .catch((error) => {
        //console.error(JSON.stringify(error))
        console.log('33',error.name, error.message)
        ask_pair_budget()
      }) 
    }
  }
...
// /////////////////////////////////////
    const clean_trades = client.ws.trades([pair], trade => {
      ...
      if ( (selling_method === "Profit") && order_id && (step === 3) && (trade.price > switch_price) ) {
        ...
        // notification(" CANCEL STOP LOSS AND GO FOR PROFIT ")
        client.cancelOrder({
          symbol: pair,
          orderId: order_id,
          recvWindow: 1000000,
      ...
      if ( (selling_method === "Trailing") && order_id && (step === 3) && (trade.price > switch_price) ) {
        ...
        // notification(" CANCEL CURRENT STOP LOSS ")
        client.cancelOrder({
          symbol: pair,
          orderId: order_id,
          recvWindow: 1000000,
        ...
        .then(() => {
          ...
          // notification(" NEW TRAILING STOP LOSS SET @ " + stop_price)
          step = 3
        })
        .catch((error) => {
          console.log(" ERROR #547 ")
      ...
      if ( (selling_method==='Profit') && order_id && (step === 5) && (trade.price < buy_price) ) {
        ...
        // notification(" CANCEL PROFIT SETTING UP STOP LOSS ")
        tot_cancel = tot_cancel + 1
        client.cancelOrder({
          symbol: pair,
          orderId: order_id,
      ...
      if ( (selling_method === "Profit") && order_id && (step === 5) && (trade.price >= sell_price) ) {
        ...
        .then( (order_result) => {
          if ( parseFloat(order_result.executedQty) < parseFloat(order_result.origQty) ) {
            ...
            reset_trade()
            report.succeed()
            notification(currency_to_buy + " 🐬 !!! WE HAVE A WINNER !!! 🐬 ")
            setTimeout( () => { ask_pair_budget(), 1000 } )
          }
        })
        .catch((error) => {
      ...
      if ( order_id && (step === 3) && (trade.price <= stop_price) ) {
        ...
        .then( (order_result) => {
          if ( parseFloat(order_result.executedQty) < parseFloat(order_result.origQty) ) {
            ...
            reset_trade()
            report.succeed()
            notification(" STOP LOSS ALL EXECUTED")
  ...
  sell_at_market_price = () => {
    ...
    // notification(" SELLING AT MARKET PRICE ")
    client.order({
      symbol: pair,
      side: 'SELL',
      type: 'MARKET',
